...
def xmgr_command(subparsers):
    ...
    xmgr_corpus = subparsers.add_parser("corpus", parents=[xmgr_shared_arguments, output_directory_argument],
                                        help="download corpus from XMGR")
    ...
    xmgr_truth = subparsers.add_parser("truth", parents=[xmgr_shared_arguments, output_directory_argument],
                                       help="download truth file from XMGR")
    xmgr_truth.set_defaults(func=truth_handler)
    # Extract questions from the usage log.
    xmgr_questions = subparsers.add_parser("questions", parents=[questions_shared_arguments],
                                           help="extract question sets from the usage log")
    xmgr_questions.add_argument("--sample-size", metavar="SAMPLE-SIZE", type=int,
    ...
    xmgr_frequency = subparsers.add_parser("frequency", parents=[questions_shared_arguments],
                                           help="get question frequencies from the usage log")
